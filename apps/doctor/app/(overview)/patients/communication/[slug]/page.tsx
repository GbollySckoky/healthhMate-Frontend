...
import { 
  ...
  onNewMessage,
  sendMessage
 } from '@/lib/constant/communicationSocket';
import { storageService } from '@/lib/constant/storage';
import { CheckCheck, Phone, Send, Video } from 'lucide-react';
...
const Page = () => {
    ...
    const { formatTime } = useGetDate();
    const rawId = String(params?.slug);
    const { message, isLoading, isError, error } = useGetAppointmentCommunicationId(rawId);
...
return (
     <div className="flex h-[100dvh] flex-col bg-gray-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-white">
                {CapitalizeName(message?.appointment.user.firstName)}{" "}
                {CapitalizeName(message?.appointment.user.lastName)}
              </h2>
                  ...
                  >
                    ...
                    <div
                      className={`mt-1 flex items-center justify-end gap-1 text-xs ${
                        isDoctor ? "text-red-200" : "text-gray-400"
                      }`}
                    >
      ...
      <div className="fixed bottom-0 w-full border-t border-gray-200 bg-white p-3.5">
        <form onSubmit={handleSubmit} className="mx-auto flex max-w-5xl items-center gap-3">
          <input
            ...
            className="h-12 rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-red-900 focus:ring-2 focus:ring-red-100 w-full"
          />
          <button
            type="submit"

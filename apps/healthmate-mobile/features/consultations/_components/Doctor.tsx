...
const getDoctorImageSource = (doctor: GetDoctor) => {
  const image = doctor.profile?.profilePicture;
...
const Doctor  = ({hospitalId}:{hospitalId: string}) => {
  ...
  const handleTabChange = (status: string) => {
    setDepartment(status === 'all' ? undefined : status);
      ...
      <div className='w-full min-w-0'>
        {isLoading ? (
          ...
          return (
            <div
              key={doctor.id}
              className="w-full p-[15px] border border-[#F2F2F2] rounded-[10px] bg-white mb-5"
            >
                ...
                <div className="flex flex-1 justify-between">
                  <div className="ml-2.5 flex-1">
                    ...
                    <p className="text-xs text-[#717680] pt-1">
                      {doctor?.department?.replaceAll("_", " ")}
                    </p>
                    <p className="text-xs text-[#717680] pt-1">
    ...
    <Wrapper>
      ...
      <Tabs defaultValue="all" className="w-full min-w-0 flex flex-col" onValueChange={handleTabChange}>
        <div className="w-full min-w-0 overflow-x-auto">
          <TabsList className="w-max max-w-none flex-nowrap text-[#717680]">
            {DEPARTMENT_OPTIONS.map((department) => (
                <TabsTrigger key={department.value} value={department.value}>{department.label}</TabsTrigger>
            ))}
          </TabsList>
        </div>
        {DEPARTMENT_OPTIONS.map((department) => (
          <TabsContent key={department.value} value={department.value} className="w-full min-w-0">{renderBody()}</TabsContent>
        ))}
      </Tabs>
    </Wrapper>
  );
